...
![queue-notnull-before](http://leocook-blog.test.upcdn.net/queue-notnull-before.png)

节点加入队列的过程如下图所示：
![queue-notnull-after](http://leocook-blog.test.upcdn.net/queue-notnull-after.png)

当队列为空时，首次加入节点会进行一些初始化，具体的操作过程可以看下图：
![queue-null-init](http://leocook-blog.test.upcdn.net/queue-null-init.png)
